...
public class LocusPassWordView extends View {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int width = MeasureSpec.getSize(widthMeasureSpec) / 5 * 4;
	...
	private void initPaints() {
		...
		selectedPaint.setStrokeWidth(dotRadius / 10);
		
		errorPaint = new Paint();
		errorPaint.setStyle(Style.STROKE);
		errorPaint.setAntiAlias(true); 
		errorPaint.setStrokeWidth(dotRadius / 10);
		
		normalPaint = new Paint();
		normalPaint.setStyle(Style.STROKE);
		normalPaint.setAntiAlias(true); 
		normalPaint.setStrokeWidth(dotRadius / 16);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (isFinish) {
			if (this.sPoints.size() == 1) {
				...
				error();
				clearPassword();
				Toast.makeText(this.getContext(), "密码过短，请重新输入！",
						Toast.LENGTH_SHORT).show();
			} else if (sPoints.size() > pwdMaxLen) {

				error();
				clearPassword();
				Toast.makeText(this.getContext(), "密码过长，请重新输入",
						Toast.LENGTH_SHORT).show();
